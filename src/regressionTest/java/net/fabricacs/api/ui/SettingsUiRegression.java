...
public final class SettingsUiRegression {
    ...
    public static int run(Path root)throws Exception{
        ...
        try{
            ...
            editor.set("count","3");tap(runtime,canvas,"Apply");check(callbacks[0]==1&&config.read().data().getInt("count")==3&&!editor.isDirty(),"successful Apply saves then notifies once");
            ...
            editor.set("count","4");tap(runtime,canvas,"Cancel");check(!window.isOpen()&&editor.value("count").equals("3")&&Arrays.equals(saved,Files.readAllBytes(config.path())),"cancel closes and discards draft only");
            ...
            editor.set("count","5");tap(runtime,canvas,"X");check(!window.isOpen()&&!editor.isDirty()&&config.read().data().getInt("count")==3,"X close discards unsaved draft");
            ...
            editor.set("count","8");JSONObject changed=new JSONObject(Files.readString(config.path()));changed.getJSONObject("data").put("count",9);Files.writeString(config.path(),changed.toString());tap(runtime,canvas,"Apply");
