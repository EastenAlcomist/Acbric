...
public final class LobbyHandshakeBridge implements AutoCloseable {
    ...
    public void welcome(JSONObject message) {
        ...
        lastFrame = -1; members = List.of(); gate.disconnect(now()); clearNativeReady();
    }
    private boolean connectionReady() {
        ...
        if (current != connection || nextGeneration != generation || readyConnection && !live) {
            gate.disconnect(now()); members = List.of(); clearNativeReady();
        }
    ...
    public boolean canReady() { return connectionReady() && (gate.canReady(now()) || gate.canStart(now())); }
    public boolean canStart() { return connectionReady() && gate.canStart(now()); }
    public void retry() { if (connectionReady()) { gate.restart(now()); syncInvalidation(); } }
    ...
    private JSONObject frame(JSONObject message) {
        ...
        for (int i = 0; i < source.length(); i++) {
            ...
            if (type.equals("strategicReady") || type.equals("strategicResumeReady")) {
                if (gate.isHost() && (!live || history || !gate.acceptReady(item.optInt("id", -1), item.optJSONObject(LobbyCodeGate.FIELD), now()))) continue;
            }
    ...
    static JSONObject stripReserved(JSONObject message) {
        if (message == null) return null;
        if (message.optString("type").equals(CodeHandshakeProtocol.TYPE)) return null;
        if (message.optString("type").equals("frame") && message.optJSONArray("messages") != null) {
            ...
            for (int i = 0; i < source.length(); i++) {
                ...
                if (!item.optString("type").equals(CodeHandshakeProtocol.TYPE)) kept.put(item);
            }
            message.put("messages", kept);
        }
        return message;
    ...
    private boolean outgoing(JSONObject message) {
        ...
        if (type.equals("strategicReady") || type.equals("strategicResumeReady")) {
            ...
            for (int i = 0; i < players.length(); i++) {
                ...
                if (proof == null || !gate.readyAccepted(player.getInt("id"))) player.put("ready", false);
            }
    ...
    public String details() {
        ...
        return text.append("\n").append(Lang.currentLocale != null && Lang.currentLocale.getLanguage().equals("zh")
                ? "点击重新检查；仅检查启动代码，不检查配置或存档。" : "Click to retry. Startup code only; settings and saves are not checked.").toString();
    }
    ...
    @Override public void close() { if (!closed) { closed = true; gate.close(); members = List.of(); connection = null; } }
}
