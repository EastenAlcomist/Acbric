# 正式外部启动入口：检查 Java 后由启动器验证发行包，并在独立实例启动游戏。
param(
...
try {
    ...
    & $javaExe '-Dfile.encoding=UTF-8' '-Dstdout.encoding=UTF-8' '-Dstderr.encoding=UTF-8' '-cp' (Join-Path $resolvedLoader '*') net.fabricacs.acbric.ExternalLauncher '--game-dir' $GameDir '--instance-dir' $InstanceDir
    $result = $LASTEXITCODE
