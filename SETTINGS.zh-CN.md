...
- `SettingsUi.window(title, editor, applied)` 可直接生成窗口。dev.19 起，取消、X、Esc 在存在草稿修改时先确认；“继续编辑”或关闭确认框保留草稿，“丢弃修改”才关闭。没有修改时直接关闭。程序 close、切换 Screen、原生覆盖层、异常或退出仍立即清理并丢弃未应用草稿。关闭普通子弹窗不丢弃父页草稿。
...
本版尚无嵌套字段路径、任意 JSON 编辑、滑块、配置自动迁移或专用 IME 面板。语言跟随游戏，窗口标题及部分固定文案在重新打开时更新。
...
应用、取消、恢复默认值和重新读取文件固定在页脚，正文单独滚动。页脚显示草稿状态或“已保存”；详细生效信息留在正文底部。保存/重读失败或保存后生效回调失败另外打开中英文消息框，关闭提示仍保留设置页；错误不会因为正文滚走而不可见。极低分辨率下页脚可单独滚动，详见 UI 契约。

示例仍无需改配置格式。展示 MOD 0.3.0 要求 dev.19，增加长文本填充按钮以测试水平滚动；旧 0.2.0 配置可继续使用。没有新增游戏逻辑、快捷键注册或网络同步接口。
