...
当前 API 构建为 **0.3.3-dev.11**。本轮增加存档规则预检查、显式旧档接纳与版本迁移，先预览再另存新档；共享规则继续在生成前固化，续局读取存档值；大厅继续执行代码检查及准备/开局门禁，双方需使用一致的框架、游戏和 MOD 代码；这不是自动状态同步，也不支持混用旧框架联机。保留运行期诊断与受管理订阅范围，继续提供配置、战役生命周期和存档/恢复集成，保留已有公开成员和事件语义。
...
标准构建通过 597 项断言，含 56 项新增存档转换检查（两项已有符号链接场景权限跳过）。两版游戏六个真实 Fabric 进程通过 136 项规则/存档/测试 MOD 检查；另四轮双客户端实验通过 54 项转换后联机检查。完整程序生成世界、GUI、跨机器和官方服务器仍待验收。
...
当前开发 API 构建为 `0.3.3-dev.11`（尚未发布）；模板最低依赖为 `>=0.3.3-dev.10`，包含需显式调用的战役数据示例。
