...
当前 API 构建为 **0.3.3-dev.5**。本轮新增按 MOD 隔离的 JSON 配置、校验、显式迁移/重载及备份原子保存；继续提供战役生命周期和存档/恢复集成，保留已有公开成员和事件语义。
...
标准构建有 203 项无界面回归，其中 62 项配置检查。用户对 dev.4 反馈暂未发现问题，未提供完整逐项清单；dev.5 GUI 与真实双机验收仍待人工完成。
