...
dev.32：新实例保存遇到目录改名拒绝时，最多尝试 6 次（等待共约 3.1 秒），持续失败保留具体诊断。已验证 Windows 文件占用后恢复；用户机器根因尚未确认。详见 INSTALLER.zh-CN.md。

dev.31：改进首次安装引导，未配置的空目录不再因点击读取而报错；已有配置保护不变。详见 INSTALLER.zh-CN.md。

dev.30：修复没有原版 MOD 时 Java MOD 列表被隐藏的问题。仅调整界面空列表判断，保留原版资源加载隔离与 Java 重启启停。详情与复验见 MOD_MANAGEMENT.zh-CN.md。
...
当前 API 为 **0.3.3-dev.32**（开发版）。框架现有事件、配套资源保护、配置与战役数据、共享规则/大厅检查、Java MOD 重启启停、公共 UI 和设置页，命令注册与游戏内开发者工具继续可用，本版修复外部模式中文安装路径音频并验证真实菜单。入口：**MOD 列表 → Acbric API → 详情 → 开发者工具 / 控制台**。
