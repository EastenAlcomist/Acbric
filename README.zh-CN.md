...
dev.22 开始外部安装重构：新增[游戏安装预检查与内部加载原型](EXTERNAL_INSTALL.zh-CN.md)。本轮 904 项标准检查、1.2.15.3 的 25 项外部加载检查、旧布局两版 142 项 ARC 检查通过。新模式尚未开放正常游戏启动，资源缓存隔离仍待实现；旧 `distZip` 仍包含本地游戏依赖，不可当作干净框架包分发。下方控制台说明及原运行/构建方法继续适用于 legacy 布局。
...
当前 API 为 **0.3.3-dev.22**（开发版）。框架现有事件、配套资源保护、配置与战役数据、共享规则/大厅检查、Java MOD 重启启停、公共 UI 和设置页，命令注册与游戏内开发者工具继续可用，本版新增外部安装预检查原型。入口：**MOD 列表 → Acbric API → 详情 → 开发者工具 / 控制台**。
