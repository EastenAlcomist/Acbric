...
dev.16 修复输入框光标宽度和文字垂直定位。间歇性点击失效尚未复现，新增有界输入诊断日志；不宣称已修复该问题。展示 MOD 0.1.1 继续使用。

dev.15 修复点击详情退回主菜单的问题；更新框架即可，展示 MOD 0.1.1 继续使用。游戏异常通常记录在 `%APPDATA%/AirshipsGame/log.txt`（自定义数据目录启用时以该目录为准）。

**dev.14 语言修复：** 已识别原生中文 `chi`，工具入口与界面跟随游戏语言。入口是展示 MOD 行的“详情”，再点“打开组件展示”。见 [UI 语言契约](UI.zh-CN.md#中英文界面dev14)。

dev.13 新增 [公共 UI 组件](UI.zh-CN.md)：原生控件、布局、滚动、焦点、模态弹窗与生命周期清理。框架 MOD 详情页使用相同 API，并展示 MOD 注册的工具入口。Java 启停仍在重启后生效；升级时同时替换启动器和 API。
...
- [存档规则预检查与显式转换](RULE_SAVE_MIGRATION.zh-CN.md)：旧档接纳、版本迁移、预览和另存保护。

- [共享战役规则](SHARED_RULES.zh-CN.md)：显式声明、全员比较、生成前固化和续局读取。
...
当前 API 构建为 **0.3.3-dev.17**。本轮增加公共 UI 组件和 MOD 工具入口，保留 Java MOD 的重启启停管理；已有存档规则预检查、显式旧档接纳与版本迁移继续支持先预览再另存新档；共享规则继续在生成前固化，续局读取存档值；大厅继续执行代码检查及准备/开局门禁，双方需使用一致的框架、游戏和 MOD 代码；这不是自动状态同步，也不支持混用旧框架联机。保留运行期诊断与受管理订阅范围，继续提供配置、战役生命周期和存档/恢复集成，保留已有公开成员和事件语义。
...
标准构建通过 698 项断言（两项已有符号链接场景权限跳过）。两版 UI 探针验证原生 input tick、缩放、点击遮蔽、工具入口和录制绘制器裁剪恢复。此前启停、存档和网络结果保留为历史；OpenGL 绘制与完整实机流程仍需验收。
...
当前开发 API 构建为 `0.3.3-dev.17`（尚未发布）；模板最低依赖为 `>=0.3.3-dev.10`，包含需显式调用的战役数据示例。
