...
当前 API 为 **0.3.3-dev.21**（开发版）。框架现有事件、配套资源保护、配置与战役数据、共享规则/大厅检查、Java MOD 重启启停、公共 UI 和设置页，本版补上命令注册与游戏内开发者工具。入口：**MOD 列表 → Acbric API → 详情 → 开发者工具 / 控制台**。
...
已通过 872 项标准回归及两套真实游戏共 464 项 UI/控制台/设置重启检查，两个已有符号链接场景因主机权限跳过。绘制终端无 GPU；Windows 剪贴板、输入法、切屏及完整玩法仍需实机验收。代码一致性和存档接入不代表任意 MOD 属性自动联机同步。
