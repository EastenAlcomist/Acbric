...
当前 API 构建为 **0.3.3-dev.10**。本轮增加共享规则声明与比较，并在新战役生成前固化，续局读取存档值；大厅继续执行代码检查及准备/开局门禁，双方需使用一致的框架、游戏和 MOD 代码；这不是自动状态同步，也不支持混用旧框架联机。保留运行期诊断与受管理订阅范围，继续提供配置、战役生命周期和存档/恢复集成，保留已有公开成员和事件语义。
...
标准构建通过 541 项无界面回归，含 64 项新增规则检查。两版游戏共 14 轮原生 Server/双 Fabric Client 隔离实验通过 204 项上层检查。另完成旧公开 API 签名对照、原始示例及新模板的真实加载/存储探针。完整 GUI、成功生成世界、跨机器和官方服务器仍待人工验收。
...
当前开发 API 构建为 `0.3.3-dev.10`（尚未发布）；模板最低依赖为 `>=0.3.3-dev.10`，包含需显式调用的战役数据示例。
...
| API 层 | `src/apiMod` | `acbric_api` —— 事件系统、入口桥、原生 MOD 界面集成、战役数据、大厅检查及 20 个 hook mixin。 |
