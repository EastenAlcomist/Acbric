...
dev.12 新增 [Java MOD 启停管理](MOD_MANAGEMENT.zh-CN.md)：在 MOD 列表选择下次启动启用/停用；核心组件保护，依赖预检查，停用条目可重新启用。**升级时同时替换启动器和 API JAR。** 标准回归现为 652 项。
...
当前 API 构建为 **0.3.3-dev.12**。本轮增加 Java MOD 的重启启停管理；已有存档规则预检查、显式旧档接纳与版本迁移继续支持先预览再另存新档；共享规则继续在生成前固化，续局读取存档值；大厅继续执行代码检查及准备/开局门禁，双方需使用一致的框架、游戏和 MOD 代码；这不是自动状态同步，也不支持混用旧框架联机。保留运行期诊断与受管理订阅范围，继续提供配置、战役生命周期和存档/恢复集成，保留已有公开成员和事件语义。
...
标准构建通过 652 项断言（两项已有符号链接场景权限跳过）。本轮两版游戏的重启链通过 114 项检查，旧启动器保护/失败状态/按钮回调另通过 56 项，6 次负向启动符合预期。dev.11 的存档与联机证据保留为历史；本轮未重跑完整联机检查。完整 GUI 绘制/鼠标交互与战役玩法仍待验收。
...
当前开发 API 构建为 `0.3.3-dev.12`（尚未发布）；模板最低依赖为 `>=0.3.3-dev.10`，包含需显式调用的战役数据示例。
