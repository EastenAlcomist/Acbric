...
当前 API 构建为 **0.3.3-dev.8**。本轮新增内部代码握手协议与状态机，尚未接入战役大厅 UI、准备和开局流程；保留运行期诊断与受管理订阅范围，继续提供配置、战役生命周期和存档/恢复集成，保留已有公开成员和事件语义。
...
标准构建有 433 项无界面回归，含 77 项握手、45 项清单、81 项订阅范围、27 项运行期诊断及原 62 项配置检查。用户对 dev.4 反馈暂未发现问题，未提供完整逐项清单；dev.8 GUI 与真实双机验收仍待人工完成。
...
当前开发 API 构建为 `0.3.3-dev.8`（尚未发布）；模板最低依赖为 `>=0.3.3-dev.6`，包含需显式调用的战役数据示例。
