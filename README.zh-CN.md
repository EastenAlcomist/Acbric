...
dev.24 完成[外部模式真实菜单与音频检查](EXTERNAL_INSTALL.zh-CN.md)，并修复中文安装路径的 OpenAL 加载。924 标准 / 36 外部 / 两版 ARC 142 检查通过；首次及缓存重启均完成真实菜单 30 帧与音频初始化，源安装 5,325 文件内容未变。正常外部入口继续等待完整战役等验收；旧 distZip 仍含游戏依赖，不作干净框架包分发。下方运行方法适用于 legacy 布局。
...
当前 API 为 **0.3.3-dev.24**（开发版）。框架现有事件、配套资源保护、配置与战役数据、共享规则/大厅检查、Java MOD 重启启停、公共 UI 和设置页，命令注册与游戏内开发者工具继续可用，本版修复外部模式中文安装路径音频并验证真实菜单。入口：**MOD 列表 → Acbric API → 详情 → 开发者工具 / 控制台**。
