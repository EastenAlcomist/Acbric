# Campaign lobby code check (dev.9)

**English** | [中文](LOBBY_HANDSHAKE.zh-CN.md)

dev.9 connects the [internal handshake](CODE_HANDSHAKE.md) to native multiplayer campaign lobbies. Checks begin automatically on entry. Pending, different, unverifiable or timed-out code blocks preparation and game start. This adds no public MOD networking API and changes no campaign save format.
...
- Local settings, effective native resources, expansions, saves and live state are neither compared nor synchronized. Self-reported identities are not authentication or anti-cheat. Matching code can still produce different state.
...
The product handles already-consumed messages at `AirshipGame.pollMessage` return, preserving unrelated message order, membership frames and native host processing. It performs no additional Client polling and consumes only `acbric:code_handshake`. Current connection, welcome metadata and live member frames define context. History cannot confirm code/preparation; duplicate or out-of-order live frames are rejected.
...
| Field | Contract |
| --- | --- |
| `v`, `channel`, `host` | Version 1, actual room ID and host player ID |
