...
适用版本：**`acbric_api 0.3.3-dev.8`**，开发版本，尚未发布稳定版。本文以本仓库源码为准；旧 0.3.2 二进制不含 `RENAME_SHIP_*`，战役数据接口要求 dev.3 或更新版本。
...
当前标准构建通过 433 项无界面回归（含 62 项配置检查）。游戏 1.2.15.2 / 1.2.14 真实 Fabric 探针各通过 10 项战役存储和 31 项生命周期/v3.1 示例和 5 项受管理事件检查，配置策略部分使用显式事件派发；不能替代 GUI 与真实联机验收。用户对 dev.4 反馈暂未发现问题，dev.8 仍待人工验收。
...
[导出与离线比较](CODE_MANIFEST.zh-CN.md) 属于内部诊断工具，不是新增公开 MOD API。433 项标准回归包含 45 项新增清单检查。CODE_MATCH 仅表示启动代码身份一致，尚未接入大厅，也未实现共享配置检查或自动同步；内部握手核心见下节。

## 16. 内部代码握手（dev.8）

见[协议与状态契约](CODE_HANDSHAKE.zh-CN.md)。这是包内框架基础设施，本轮有 77 项新增回归，不是公开 MOD 网络接口。两套游戏快照的真实 Server/Client 探针已调用验证；正常游戏启动尚不执行。下一步是大厅 UI、准备/开局阻断及房间协调接入。
