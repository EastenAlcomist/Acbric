...
2026-09-26 提交验收：用户确认新版安装包在此前持续保存失败的机器上成功，要求提交本轮 dev.30–32（空原版 MOD 列表、首次配置引导、Windows 发布重试及版本化包名）。此反馈支持短暂占用假设，但成功路径未记录重试次数/占用进程，也未控制目录等环境变量，不能断定杀毒软件或权限根因。沿用 1011 标准检查、dev.30 真实列表 26 项及 dev.32 Windows 真实句柄测试，版本化包内容扫描 PASS；本轮只补用户验收记录，不重复未改代码的测试。纳入 dev，未要求推送。下方未提交状态为历史。

2026-09-26 发行命名：按用户要求 externalDistZip 自动读取 fabric.mod.json，输出 Acbric-external-<version>.zip（当前 0.3.3-dev.32）；同步真实发行验收脚本和中英文使用说明。只调整文件命名，不提高版本号；旧无版本 ZIP 不再作为交付链接，未删除旧测试证据/包。

dev.32 收尾：全量 16 套件 1011 项通过（build/setup-publication-all.log）。本轮仅改安装保存层并同步发行版本，未重跑真实游戏菜单或对方机器；远端持续失败请收集完整新日志，不能宣称已彻底修复。包由 externalDistZip 执行游戏内容排除扫描。

2026-09-26 dev.31（未提交）：用户反馈 INSTANCE_CONFIG_INVALID 堆栈定位到 InstallerPanel 的读取已有实例按钮，而非 Steam 识别或 preview 调用。首次配置前空目录没有 instance.json 属正常状态；新增 loadSaved 只对此返回空结果并显示双语流程，已有管理目录配置缺失/损坏/过大和旧文件仍拒绝接管。首行按 Steam→检查→保存排列，读取按钮更名“读取已保存配置”。缺失和超过 64 KiB 分别诊断，公开 API 无新增。定向 external 128 项、全量 16 套件 1001 项通过，包含 13 项新增；中英文面板离屏绘制已检查，未复现对方机器现场/未重跑游戏实机。本次源码、日志和面板截图记录在 build/installer-guidance-* 与 regression-sandbox/run-13802467681811654308；保留上一轮 dev.30 列表修复。未提交/推送，未覆盖玩家副本。下方为历史。

2026-09-26 dev.30（未提交）：修复原版 MOD 为空时 Java 行被整个列表空态分支隐藏的问题。仅 ModsScreen.render 的 Mod.getAvailableMods 调用转到 FabricModListBridge.modsForListVisibility；不全局改可用性，不移除 preemptedBy 隔离，不加虚假原版 MOD。988 标准检查、实际 1.2.15.3 两进程共 26 项列表检查通过，旧 dev.29 同场景稳定失败；源游戏 5,325 文件未改。测试 --java-only 排除原版与配套资源，覆盖刷新、混合/空态和禁用重启。编译依赖与研究安装 ModsScreen.class 哈希相同；已做 ASM 目标计数与真实 Knot 注入/绘制。BUILDING 引用的 Ac source/tools/acbric.cmd 在当前工作区不存在，未声称运行该工具。证据：99-研究工具/Acbric空原版MOD列表修复-dev30-20260926。未重跑战役/联机，未自动提交/推送或覆盖玩家副本；下方为历史。
...
2026-09-26 dev.32（未提交）：用户按正常顺序检查通过，保存时报 Windows AccessDenied 目录改名失败；截图中仅初始化空目录/锁文件，远端根因未知。新增内部 SetupDirectoryPublisher，仅对首次新实例的原子目录改名拒绝重试，6 次/等待 3.1 秒，每轮目标检查、无复制回退、可中断；保留路径/状态和首末异常。InstanceSetup 清理失败不再遮盖主异常。定向 external 138 项通过，新增 10 项含真实 Windows NOSHARE_DELETE 句柄恢复/持续拒绝/冲突/中断；不把该受控复现当作对方机器根因确认。证据 build/setup-publication-test.log，未改玩家文件、未提交/推送；保留 dev.30–31 改动。下方为历史。
...
2026-09-26 提交核验：用户授权将 dev.26–dev.28 安装器与目录调整纳入 dev 分支。58 个变更文件与 dev.28 已验收快照逐项哈希一致，发行包匹配记录；本次仅补此提交说明，沿用 977 标准、36 外部加载、两次真实菜单及路径路由/扫描验证。未要求推送，保留远端分歧，不自动合并。下一步讨论框架更新与失败回退，尚未实现；下方未提交状态为历史。
...
2026-09-26 dev.25 提交核验：用户要求先提交并解释后续数据迁移，本轮不实现迁移。源码/验证文档与已验收快照逐项哈希一致，交付 ZIP、启动器、API、模板构建及运行时均匹配记录；仅维护提交状态说明，沿用 935 标准、36 外部加载和发行/真实菜单验收，不重复执行未改代码。包含此前未提交的战役与媒体验收夹具；不纳入游戏资料、构建包或工作区研究记录。未要求推送，保留远端分歧，不自动合并。
