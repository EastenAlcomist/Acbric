# Acbric 安装、启动与更新（dev.32）


dev.32 保存说明：首次配置会先写入临时目录，再一次改名为 `acbric-launcher`。Windows 拒绝改名时最多尝试 6 次，共等待约 3.1 秒；不会改为部分复制。持续失败时提供 `SETUP_PUBLISH_DENIED` 日志，保留首末异常及源/目标状态；目标已经存在或无法确认不存在时停止，不覆盖。请发送界面显示的完整日志；不要删除实例或手动建立空配置。失败清理若也被占用，会保留残留临时文件并记入同一日志，需保留现场。`.acbric-instance.lock` 是可复用的锁文件，其存在不代表仍有进程占用。
[English](INSTALLER.md)

解压 `Acbric-external-<version>.zip`，打开其中的 `Acbric` 文件夹。**Setup.cmd、Start Acbric.cmd 和 mods 现在位于同一层。**
...
首次安装不需要点击“读取已保存配置”（旧版叫“读取已有实例”）。Steam 查找只填写游戏目录；`acbric-launcher/instance.json` 在保存成功后才生成。dev.31 在未配置的空目录上点击读取时只显示安装步骤，不创建文件，也不清空游戏路径。若已经有管理目录或其他文件，却缺失、损坏或超过 64 KiB 的实例配置，仍会报错并保留文件；请核对原实例位置，不要手工创建空 JSON 来跳过检查。
...
1. 关闭游戏和 Setup，保留从可信来源取得的新版 `Acbric-external-<version>.zip`，不要直接覆盖解压。
...
解压新包后运行新位置的 Setup，选择原来的实例，点击“读取已保存配置”，确认游戏位置，检查并保存。原存档与配置保留。移动整套框架时一起移动整个 Acbric 文件夹，其中已经包含 mods；游戏或外置实例移动后也应重新选择路径并保存。
